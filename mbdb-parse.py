...
def getstring(data, offset):
    ...
    # MBDB strings are typically UTF-8; surrogateescape preserves odd bytes without lossy replacement
...
def process_mbdb_file(filename):
    ...
    while offset < len(data):
        ...
        sha_in = (fileinfo['domain'] + '-' + fileinfo['filename']).encode('utf-8', errors='surrogateescape')
        #fileinfo['fileID'] = hashlib.sha1(fileinfo['domain'] + '-' + fileinfo['filename']).hexdigest()
        fileinfo['fileID'] = hashlib.sha1(sha_in).hexdigest()
...
def fileinfo_str(f):
    ...
    if (f['mode'] & 0xE000) == 0xA000: type = 'l' # symlink
        ...
        print("Unknown file type %04x for %s" % (f['mode'], f.get('fullpath', '?')), file=sys.stderr)
